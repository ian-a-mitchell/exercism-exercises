...
class ForthInterpreter:
    ...
    def __init__(self):
        """
        Creates a unique interpreter object to use when evaluating later 
        supplied code/data stacks.
        ...
        Returns
        -------
        None--object is created, that's all.
        """
    ...
    def add_user_word(self, definition: str) -> None:
        """
        Updates the interpreter's translation dictionary with user-defined
        keywords. NO value is returned--this is a side-effect only method!

        Parameters
        ----------
        definition : str
            A single user-defined keyword defiction; a string of format
            ": word def ;".
        ...
        ------
        ValueError
            ValueError raised if a forbidden operation (mostly trying to use
            a number as a keyword) is attempted.
        """
...
def evaluate(input_data: list[str]) -> list[int]:
    ...
    which the latter is sent to that interpreter for interpretation.
    ...
    my_interpreter = ForthInterpreter()
    
    for word in user_words:
        my_interpreter.add_user_word(word)
