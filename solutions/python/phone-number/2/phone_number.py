...
class PhoneNumber:
    ...
    def _validate_number(number: str) -> str:
        ...
        if any(char in ascii_letters for char in non_digits):
            raise ValueError("letters not permitted")
            
        forbid_punc = set(punctuation) - PhoneNumber._ALLOW_PUNC
        if any(char in forbid_punc for char in non_digits):
    ...
    def pretty(self) -> str:
        ...
        output = "(" + self.area_code + ")" + "-" + self.exchange_code
        output += "-" + self.subscriber
