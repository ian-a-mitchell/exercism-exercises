...
def say(number: int) -> str:
    ...
    for power in POWER_MARKERS:
        if red_num // power > 0:
            chunk_num.append((int(red_num // power), POWER_MARKERS[power]))
            red_num = red_num % power
            
    chunk_num.append((int(red_num), ""))
    
    output = []
    
    for num, suffix in chunk_num:
        if num > 0:
            output.append(conv_hundreds(num))
