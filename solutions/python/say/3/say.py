...
def conv_triplets(number: int) -> str:
    """
    ...
    if number // power > 0:
        output.append(NUMS[int(number // power)])
        output.append(NUMS[power])
    ...
    if number > 0:
        if number < 21:
            output.append(NUMS[number])
        else:
            tens, ones = divmod(number, 10)
            tens = int(tens * 10)
            tail = ""
            if ones > 0:
                tail = NUMS[tens] + "-" + NUMS[ones]
            else:
                tail = NUMS[tens]
            output.append(tail)
...
def say(number: int) -> str:
    ...
    for power, suffix in BASES.items():
        if red_num // power > 0:
            power_triplet, red_num = divmod(red_num, power)
            chunk_num[power_triplet] = suffix
            
    if red_num > 0:
        chunk_num[int(red_num)] = ""
            
    output = [conv_triplets(num) + suffix for num, suffix in chunk_num.items()]
