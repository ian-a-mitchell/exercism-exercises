...
def build_verse(verse_num: int) -> list:
    ...
    first_end = f'bottl{"es" if verse_num != 1 else "e"} hanging on the wall,'
    
    mid_line = 'And if one green bottle should accidentally fall,'
    
    last_start = f'There\'ll be {INT_TO_STR[verse_num - 1]} green bottl'
    last_end = f'{"es" if (verse_num - 1) != 1 else "e"} hanging on the wall.'
