""" A set of methods that build the lyrics to the children's song
    ...
    Repeate starting at n - 1, until n - 1 is equal to zero. The only variation
    comes when n or n - 1 is one or zero; in the first case, "bottles" is
    replaced with "bottle," and in the second "zero" is replaced with "no".
    
    The necessary functionality is split between three methods:
        1: bottle determines whether "bottles" or "bottle" is the correct
        form of the word to use for a given verse/line;
        2: build_verse creates all of the lines for a single verse;
        3: recite handles validation and combining all of the requested
        verses.
