...
class Node:
    ...
    def __hash__(self):
        """
        Defines a hash value to remove an error message from the linter...
        """
...
class Edge:
    ...
    def __hash__(self):
        ...
        Defines a hash value to remove an error message from the linter...
        """
        return hash((self.src, self.dst, self.attrs))
...
class Graph:
    ...
    @staticmethod
    def _val_data(data: list[tuple]) -> None:
        """
    ...
    @staticmethod                
    def _val_item(data: tuple[str]) -> None:
        """
        ...
        if data:
            for item in data:
                try:
                    ...
                    raise TypeError("Graph item incomplete") from exc
                # Contrary to linter reports this is an optional keyword
                # in try...except blocks for code that should be executed if
                # an error is *not* raised, and is therefore perfectly needed
                # here (so that prefx can still be accessed).
