...
def solve():
    ...
    def validate_hobbies(nations, colors, drinks, pets, hobbies):
        """ 
    ...
    for nation in permutations(nat_idx):
        nation = [norwegian] + list(nation)
        for color in permutations(color_idx):
            color = [yellow, blue] + list(color)
            if validate_colors(nation, color):
                for pet in permutations(pet_idx):
                    pet = [pet[0]] + [horse] + list(pet[1:])
                    if known3(nation, pet):
                        for drink in permutations(drink_idx):
                            drink = list(drink[0:2]) + [milk] + list(drink[2:])
                            if validate_drinks(nation, color, drink):
                                for hobby in permutations(hobby_idx):
                                    hobby = [painter] + list(hobby)
                                    if validate_hobbies(nation, color, drink, pet, hobby):
                                        houses = [
                                            {
