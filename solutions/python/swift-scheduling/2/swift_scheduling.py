...
def set_month(date, key_year, key_month, key_day):
    
    output = date.replace(year = key_year, month = key_month, day = key_day)
...
def delivery_date(start, description):
    ...
    match description:
        ...
        case description if description[-1] == "M":
            ...
            if cur_month < key_month:
                output = set_month(output, output.year, key_month, key_day)
            else:
                output = set_month(output, output.year + 1, key_month, key_day)
        ...
        case description if description[0] == "Q":
            ...
            if key_month in (3, 12): # Handle March and December
                key_day = 31
