...
def grep_file(pattern: str, flags: list[str], file: str) -> list[str]:
    ...
    r_pattern = fr"^{pattern}$" if _WHOLE_LINE in flags else pattern
    
    re_flags = re.IGNORECASE if _IGNORE_CASE in flags else 0
    ...
    for idx, line in enumerate(my_text):
        ...
        if bool(patt_match) ^ (_INVERT_MATCH in flags):
            new_line = f"{idx + 1}:" + line if _NUM_LINES in flags else line
    ...
    for file in files:
        ...
        if next_output:
            if _LIST_FILE not in split_flags:
                prefix = file + ":" if len(files) > 1 else ""
