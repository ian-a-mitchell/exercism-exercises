...
class PokerHand:
    ...
    def _ns_score(me, you, num: int):
        ...
        my_ns = me._get_n_set(num)
        your_ns = you._get_n_set(num)
    ...
    def __init__(self, hand: str) -> None:
        ...
        flush = len(set([card[1] for card in self.proc_hand])) == 1
        
        hand_type = [self._straight() and flush, self._get_n_set(4),
                     self._full_house(), flush, self._straight(), 
                     self._get_n_set(3), len(self._get_n_set(2)) == 2,
                     len(self._get_n_set(2)) == 1, True]
        
        self.category = len(hand_type)
        
        for idx, cat in enumerate(hand_type):
            if cat and not self.category < len(hand_type):
                self.category = idx
    ...
    def _full_house(self) -> bool:
        ...
        return self._get_n_set(3) and len(set(self.values)) == 2
    
    def _get_n_set(self, num: int) -> list[str]:
        """ 
