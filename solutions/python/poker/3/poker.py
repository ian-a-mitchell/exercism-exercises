...
class PokerHand:
    ...
    @staticmethod
    def _straight_score(this_hand, that_hand):
        """ 
        Finds the winner between the PokerHands this_hand and that_hand if they are both
        ...
        i_low_st = (set(this_hand.values) == set(PokerHand._LOW_STRAIGHT))
        you_low_st = (set(that_hand.values) == set(PokerHand._LOW_STRAIGHT))
        ...
        if not output:
            output = PokerHand._high_card_score(this_hand, that_hand)
                        
        return output
    
    @staticmethod
    def _ns_score(this_hand, that_hand, num: int):
        """ 
        Finds the winner between the PokerHands this_hand and that_hand if they are both
        ...
        these_ns = this_hand.get_n_set(num)
        those_ns = that_hand.get_n_set(num)
        
        return PokerHand._subhand_compare(this_hand, that_hand, these_ns, those_ns)
    
    @staticmethod
    def _high_card_score(this_hand, that_hand):
        """ 
        Finds the winner between the PokerHands this_hand and that_hand using the high card
        ...
        return PokerHand._subhand_compare(this_hand, that_hand, this_hand.values, that_hand.values)
    
    @staticmethod
    def _subhand_compare(this_hand, that_hand, these_cards, those_cards):
        """ 
        ...
        ----------
        this_hand: PokerHand
            The PokerHand from which these_cards came from.
        that_hand: PokerHand
            The PokerHand from which those_cards came from.
        these_cards: list[str]
            A list of card values from the PokerHand this_hand that need to be scored
            against a corresponding list from the PokerHand that_hand.
        those_cards: list[str]
            A list of card values from the PokerHand that_hand that need to be scored
            against a corresponding list from the PokerHand this_hand.
        ...
        for these_card, those_card in zip(these_cards, those_cards):
            if not winner:
                these_value = PokerHand._VALUES[these_card]
                those_value = PokerHand._VALUES[those_card]
                if these_value > those_value:
                    winner = these_cards
                elif these_value < those_value:
                    winner = those_cards
                
        if winner == these_cards:
            output = this_hand
        elif winner == those_cards:
            output = that_hand
            
        return output
    
    _HAND_FUNCS = {
            0: lambda this_hand, that_hand: PokerHand._straight_score(this_hand, that_hand),
            1: lambda this_hand, that_hand: PokerHand._ns_score(this_hand, that_hand, 4),
            2: lambda this_hand, that_hand: PokerHand._ns_score(this_hand, that_hand, 3),
            3: lambda this_hand, that_hand: PokerHand._high_card_score(this_hand, that_hand),
            4: lambda this_hand, that_hand: PokerHand._straight_score(this_hand, that_hand),
            5: lambda this_hand, that_hand: PokerHand._ns_score(this_hand, that_hand, 3),
            6: lambda this_hand, that_hand: PokerHand._ns_score(this_hand, that_hand, 2),
            7: lambda this_hand, that_hand: PokerHand._ns_score(this_hand, that_hand, 2),
            8: lambda this_hand, that_hand: PokerHand._high_card_score(this_hand, that_hand)
    }
    ...
    def __init__(self, hand: str) -> None:
        ...
        self.these_score = lambda other: PokerHand._HAND_FUNCS[self.category](self, other)
