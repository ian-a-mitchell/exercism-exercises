def factorization(number):
    ...
    DOES NOT provide the number itself as a factor!
    ...
    if number > 2:
    # 2 is only divisible by 1 and skipping it solves certain issues.
...
def aliquot_sum(number):
    ...
    return sum(factorization(number))
...
def classify(number):
    ...
    output = "deficient" if (x < 0 or number == 1) else "abundant" if x > 0 else "perfect"
