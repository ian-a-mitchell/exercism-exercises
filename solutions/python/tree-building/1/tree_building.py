...
def BuildTree(records):
    ...
    Right now it uses nested looping to first find the parent_id for a node and
    then find the parent node corresponding to that node to insert that node
    into, which is probably not the most efficient.
    
    Builds leaves-up to avoid having to walk the tree to find branch (but not
    leaf) nodes.
    ...
    # This ensures the nodes are created in order of their parent_id, largest
    # first; that is, that the leaf nodes come first and then branches and
    # lastly the root node.
    records.sort(key = lambda record: record.parent_id, reverse = True)
