...
def chunk_input(input_grid):
    ...
    output = [chunk_line(input_grid[i:i+4]) for i in range(0,len(input_grid),4)]
...
def chunk_line(raw_line):
    ...
    for i in range(0, len(raw_line[0]), 3):
        grid = []
        for j in range(len(raw_line)):
            grid.append(raw_line[j][i:i+3])
...
def translate(grid_square):
    ...
    output = ''
    if grid_square in TRANS: 
        output = TRANS[grid_square]
    else: 
        output = '?'
